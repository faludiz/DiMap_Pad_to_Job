    ...
    for i in range(len(info)):
        ...
        if len(sdata) > 1:
            if sdata[0] == "deviceModel":
                device_model = sdata[1]
            if sdata[0] == "deviceSn":
                device_sn = sdata[1]
    return [device_model, device_sn]
...
def save_job(folder, projname):
    ...
    work_id = get_work_id(folder, projname)
    con = sqlite3.connect(folder + "/" + "surveyDatabase.db")
    cur = con.cursor()
    sql = 'SELECT * FROM "Point" WHERE "work_id" IS "' + work_id + '";'

    station_name = ""
    last_station_name = ""

    job = open(projname + ".job", "w")
